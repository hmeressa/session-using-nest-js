import { BadRequestException,NotFoundException, Body, Controller, Delete, Get, Param, Patch, Post } from '@nestjs/common';
...
export class PermissionController {
    ...
    async createPermission(@Body() permissionDto: PermissionDto): Promise<any>{
        const isExist = await this.permissionService.getPermissionByName(permissionDto.name);
        if (isExist) {
    ...
    @Patch(":id")
    async updatePermission(@Param('id') id: string, permissionUpdateDto: PermissionUpdateDto): Promise<any>{
        ...
        return await this.permissionService.updatePermission(id, permissionUpdateDto);
    }
}
